...
# Initialize variables for orientation, speed, and position
...
def correct_drift(orientation, gravity, magnetometer):
    # Correct pitch and roll using gravity
    g = gravity / np.linalg.norm(gravity)
    pitch_correction = np.arctan2(g[1], g[2])
    roll_correction = np.arctan2(-g[0], np.sqrt(g[1]**2 + g[2]**2))

    # Correct yaw using magnetometer
    # Assuming magnetometer is calibrated and gives heading in degrees
    yaw_correction = np.radians(magnetometer)

    return np.array([roll_correction, pitch_correction, yaw_correction])


# Additional function for numeric integration
def integrate(data, delta_time):
    integral = np.cumsum(data) * delta_time
    return integral
...
def main(verbose=False, gui=False):
    ...
    if gui:
        try:
            ...
            def data_gen():
                global orientation, speed, position
                t = 0
                        ...
                        try:
                            ...
                            if len(sensor_values) == len(columns):
                                ...
                                sensor_values[2] = kf_z.update(sensor_values[2])  # Z Orientation
                                 # Gyroscope readings (assuming they are in degrees per second)
                                gyro_readings = np.array([sensor_values[3], sensor_values[4], sensor_values[5]])
                                gyro_readings = np.radians(gyro_readings) # Convert to radians

                                # Update orientation
                                orientation += gyro_readings * delta_time


                                # # Extract gravity and magnetometer data
                                # gravity_data = np.array([sensor_values[16], sensor_values[17], sensor_values[18]])
                                # magnetometer_data = sensor_values[20]  # Assuming this is the heading

                                # # Apply drift correction
                                # corrections = correct_drift(orientation, gravity_data, magnetometer_data)
                                ...
                                accel_readings = np.array([sensor_values[6], sensor_values[7], sensor_values[8]])

                                # Transform accelerometer readings to world frame
                                xfm_accel_readings = R.dot(accel_readings)
                                # Print accelerometer readings
                                print("Accelerometer:", xfm_accel_readings)
                                # Integrate the acceleration to get speed
                                speed += integrate(xfm_accel_readings, delta_time)
                                # Print the speed
                                print("Speed:", speed)
                                # Integrate the speed to get position
                                position += integrate(speed, delta_time)
                                # Make a copy of position but in cm
                                position_cm = position * 100
                                # Print the position
                                print("Position:", position_cm)
